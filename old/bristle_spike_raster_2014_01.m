...
if manual == 1
[spike_locs] = simple_spike_extractor_07_2014(filtered_data(spikes.spike_count_range)', bristle_1(spikes.spike_count_range)',piezo(spikes.spike_count_range));
...
% if any(diff(incs) < 5); incs(diff(incs) < 10) = [];end %%make sure incs are not too close
...
if any(diff_incs < 200) %% for cases where the light stimulus has a rapid duty cycle- use only the onset and offset
