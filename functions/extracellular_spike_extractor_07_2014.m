function [detected_spike_locs, spikesBelowThresh, spikeTemplate, spikeDistThreshold] = extracellular_spike_extractor_07_2014(data,...
...
threshold_min = 2; threshold_max = 30;
...
while isempty(spikes_flag)          % Wait while the user does this.
   ...
   spikes_flag = input('\n If there are no spikes, press 1...');
    ...
    spikeTemplate(hh,:) = data(round(template_center-(0.5*spikeTemplateWidth)):round(template_center+(0.5*spikeTemplateWidth)));
    catch err
